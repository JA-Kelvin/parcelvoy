...
const extractGlobalAttributes = (mjmlElement: Element): Record<string, Record<string, string>> => {
    ...
    const processAttributeElement = (element: Element) => {
        const tagName = element.tagName.toLowerCase()

        // Only process MJML attribute definition tags
        if (tagName.startsWith('mj-')) {
        ...
        for (let i = 0; i < element.children.length; i++) {
            processAttributeElement(element.children[i])
        }
    }
