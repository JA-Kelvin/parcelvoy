...
const editorReducer = (state: HistoryState, action: EditorAction): HistoryState => {
    ...
    switch (type) {
        case 'ADD_ELEMENT': {
            ...
            const updatedElements = addElementRecursive(state.present, element, parentId, index)
            return {
        ...
        case 'UPDATE_ELEMENT': {
            ...
            const updatedElements = updateElementRecursive(state.present, elementId, attributes, content)
            return {
                present: updatedElements,
                history: [...state.history.slice(-49), state.present],
                future: [],
        ...
        case 'DELETE_ELEMENT': {
            ...
            const updatedElements = deleteElementRecursive(state.present, elementId)
            return {
                present: updatedElements,
                history: [...state.history.slice(-49), state.present],
                future: [],
...
const EnhancedMjmlEditorDndKit: React.FC<EnhancedMjmlEditorProps> = ({
    ...
    // Optimized element delete handler with requestAnimationFrame for better performance
        ...
        try {
            ...
            requestAnimationFrame(() => {
                try {
                    ...
                    // Delete the element using our recursive function - wrap in try/catch for safety
                    let updatedElements: EditorElement[] = []
                    try {
                        updatedElements = deleteElementRecursive(safeElements, elementId)
                    } catch (deleteError: unknown) {
                        console.error('Error in deleteElementRecursive:', deleteError)
                        // Fallback to original elements if deletion fails
                        updatedElements = safeElements
                        toast.dismiss(loadingToast)
                        toast.error('Failed to delete element: ' + ((deleteError as Error)?.message || 'Unknown error'))
                        return
                    }

                    // Use LOAD_TEMPLATE instead of DELETE_ELEMENT for better stability
                    // Wrap in requestAnimationFrame to ensure UI updates properly
                    requestAnimationFrame(() => {
                        // Update state
    ...
    // Clear canvas handler - optimized with requestAnimationFrame
    const handleClearCanvas = useCallback(() => {
        try {
            // Use confirmation dialog
            if (window.confirm('Are you sure you want to clear the canvas? This action cannot be undone.')) {
                // Show loading toast
                const loadingToast = toast.loading('Clearing canvas...')
                ...
                requestAnimationFrame(() => {
                    try {
                        ...
                        // Use nested requestAnimationFrame to ensure UI updates properly
                        requestAnimationFrame(() => {
                            try {
                                // Update state with default elements
                                ...
                                toast.dismiss(loadingToast)
                                toast.error('Failed to clear canvas: ' + ((stateError as Error)?.message || 'Unknown error'))
                            }
                        })
                    } catch (structureError: unknown) {
                        console.error('Error creating default structure:', structureError)
                        toast.dismiss(loadingToast)
                        toast.error('Failed to create default structure: ' + ((structureError as Error)?.message || 'Unknown error'))
                    }
                })
            }
        } catch (error) {
    ...
    }, [editorState.templateId, editorState.selectedElementId])

    // Save functionality - optimized with requestAnimationFrame
    const handleSave = useCallback(async () => {
        ...
        requestAnimationFrame(async () => {
            try {
                ...
                // Split heavy operations across animation frames
                try {
                    // Generate MJML string (potentially heavy operation)
                    const mjmlString = editorElementsToMjmlString(safeElements)

                    // Generate HTML in the next frame
                    try {
                        ...
                        try {
                            ...
                            toast.dismiss(loadingToast)
                            toast.error('Failed to save template: ' + ((saveError as Error)?.message || 'Unknown error'))
                        }
                    } catch (htmlError: unknown) {
                        console.error('Error generating HTML:', htmlError)
                        toast.dismiss(loadingToast)
                        toast.error('Failed to generate HTML: ' + ((htmlError as Error)?.message || 'Unknown error'))
                    }
                } catch (mjmlError: unknown) {
                    console.error('Error generating MJML:', mjmlError)
                    toast.dismiss(loadingToast)
                    toast.error('Failed to generate MJML: ' + ((mjmlError as Error)?.message || 'Unknown error'))
                }
            } catch (error) {
    ...
    // Import MJML functionality - optimized with requestAnimationFrame
                        ...
                        requestAnimationFrame(() => {
                            try {
                                ...
                                try {
                                    ...
                                    toast.dismiss(loadingToast)
                                    toast.error('Failed to update template: ' + ((templateError as Error)?.message || 'Unknown error'))
                                }
                            } catch (stateError: unknown) {
                                console.error('Error updating editor state:', stateError)
                                toast.dismiss(loadingToast)
                                toast.error('Failed to update editor: ' + ((stateError as Error)?.message || 'Unknown error'))
                            }
                        })
                    } catch (htmlError: unknown) {
                        console.error('Error generating HTML:', htmlError)
